...
# ╔═╡ 0c17492b-ddf1-4079-b0b1-064fa5695f5e
	S₀ᵘ = (stable = 34.551, cabbeling = 34.567, cabbeling2 = 34.559)
...
# ╔═╡ 1700159d-810a-4ff8-93d6-49d4987a60e3
md"""
## Setting salinity perturbations over a range of depths
"""

# ╔═╡ c1f4e8cd-d944-4058-b059-1c9480d69546
begin
	depths = find_depth(model, [INTERFACE_LOCATION + 0.02, INTERFACE_LOCATION - 0.02])
	initial_noise_multiple = SalinityNoise(depths, fill(1e-4, length(depths)))
	dns_ms = TwoLayerDNS(model_mp, profile_function_mp, initial_conditions, initial_noise = initial_noise_multiple)
	set_two_layer_initial_conditions!(dns_ms)
	visualise_initial_conditions(dns_ms, 1, 1)
end

# ╔═╡ b3b7472e-c936-4a80-bd59-e7aca46e701d
visualise_initial_density(dns_ms, 1, 1, 0)

# ╔═╡ 6243b463-72c2-43e1-b9b8-fbe4289246aa
let
	S_star, Θ_star = 34.7, 0.5
	T_profile = interior(dns_ms.model.tracers.T, 1, 1, :)
	S_profile = interior(dns_ms.model.tracers.S, 1, 1, :)
	N = 2000
	S_range, Θ_range = range(34.52, 34.72, length = N), range(-2, 1, length = N)
	S_grid, Θ_grid = ones(N) .* S_range', ones(N)' .* Θ_range
	ρ = gsw_rho.(S_grid, Θ_grid, 0)
